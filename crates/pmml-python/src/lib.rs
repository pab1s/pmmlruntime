...
//! In v2 it will expose `#[pyclass] InferenceSession` wrapping `pmml_session::Session` with
...
//! # Thread safety
//!
//! `PyO3` requires `Send` for `#[pyclass]`. `pmml_session::Session` is already `Send+Sync`, so `InferenceSession` will be too.
...
//! assert_eq!(pmml_python::placeholder(), ());
//! ```
