...
//!
//! - `PmmlEnv` holds `Arc<EnvInner>` (`pmml_session::PmmlEnv`) and is `Send` + `Sync`. It is reference-counted;
//!   multiple `PmmlSession`s can share one `PmmlEnv`.
//! - `PmmlSession` holds `Box<Session>` (`pmml_session::Session`) and is `Send` + `Sync` (scoring is `&self`).
...
/// Opaque handle for the global environment (`Arc<EnvInner>` inside `pmml_session::PmmlEnv`).
///
...
/// Opaque handle for a scoring session (`Box<Session>` inside `pmml_session::Session`).
///
...
/// In v2 it will read the file, `pmml_xml::unmarshal` → `verify` → `lower` → `Session::from_file` and return rich errors.
///
/// # Parameters
///
