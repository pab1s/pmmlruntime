...
//! child structs. Cold-path parsing (`pmml-xml` → [`pmml_xml::RawPmml`]) is
//! lowered once via [`crate::lower::lower()`] into this representation.
//!
//! Key design choices:
//! - Stable [`pmml_core::FieldId`] / [`pmml_core::SymbolId`] assigned by [`crate::Interner`].
...
///
/// ```
/// use pmml_core::{FieldId, DataType, OpType};
/// use pmml_ir::ir::{FieldMeta, OutlierTreatment, InvalidValueTreatment, MissingValueTreatment};
///
/// let meta = FieldMeta {
///     field_id: FieldId(0),
///     name: "age".into(),
...
pub struct FieldMeta {
    /// Stable numeric identity for this field, dense per [`crate::Interner`].
    ///
    /// Used as index into the hot-path `&[pmml_core::Value]` array. See
    ...
    ///
    /// Determines whether a value is stored as [`pmml_core::Value::Discrete`]
    /// or [`pmml_core::Value::Continuous`] on the hot path. See [`DataType`].
...
pub enum OutlierTreatment {
    ...
    AsIs,
    /// Treat outlying values as [`pmml_core::Value::Missing`] (`asMissingValues`).
    AsMissingValues,
...
/// string via `parse_invalid_treatment` in [`mod@crate::lower`]; default is
...
/// `"add" | "+" → Add`) via `resolve_builtin` in [`mod@crate::lower`].
...
/// See [`Ir::extensions`] and [`crate::verify::verify_raw`].
#[derive(Debug, Clone)]
...
///
/// Produced once by [`crate::lower::lower()`] and held as `Arc<Ir>` in
...
impl Ir {
    ...
    ///
    /// Determines the length of the hot-path `&mut [pmml_core::Value]` array
