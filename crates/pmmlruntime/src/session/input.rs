...
///
/// ```
/// use pmmlruntime::session::input::string_to_value;
/// use pmmlruntime::base::{FieldId, field::{DataType, OpType}, Value, SymbolId};
...
pub fn string_to_value(
    ...
    data_type: Option<crate::base::field::DataType>,
    op_type: Option<crate::base::field::OpType>,
    ...
    if let Ok(f) = s.parse::<f64>() {
        if let Some(fid) = field_id {
            ...
            if let (Some(dt), Some(op)) = (data_type, op_type) {
                if dt == crate::base::field::DataType::String
                    || op == crate::base::field::OpType::Categorical
                {
