...
//! calls [`apply_mining_schema`] before [`crate::engine::transform::eval_derived_fields`].
//! The dense array is then read by [`crate::engine::predicate::eval_predicate`] and every
//! `evaluate_*` in [`crate::engine::models`].
//!
//! # Invariants
//!
...
/// `symbol_names` maps [`crate::base::SymbolId`] to its string rendering for
...
/// [`crate::base::SymbolId`]; callers without an interner receive a placeholder `SymbolId(0)`.
///
...
fn parse_replacement(
    s: &str,
    meta: &FieldMeta,
    interner: Option<&mut dyn FnMut(&str) -> crate::base::SymbolId>,
) -> Value {
    ...
    match meta.data_type {
        DataType::Double | DataType::Float | DataType::Integer => {
            ...
            Value::Discrete(crate::base::SymbolId(0))
        }
        _ => {
...
mod tests {
    ...
    fn invalid_as_missing() {
        ...
        meta.values = vec![crate::base::SymbolId(1), crate::base::SymbolId(2)];
        let schema = MiningSchemaIr {
            active_fields: vec![FieldId(0)],
            target_field: None,
        ...
        let mut map = HashMap::new();
        map.insert(FieldId(0), Value::Discrete(crate::base::SymbolId(99))); // invalid
        let mut values = vec![Value::Missing; 1];
        apply_mining_schema(&schema, &map, &mut values).unwrap();
        assert_eq!(values[0], Value::Missing);
    }
