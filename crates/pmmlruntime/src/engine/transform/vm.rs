...
//!
//! This module is the `pmml-ir` bytecode interpreter. Each [`DerivedFieldIr`](crate::ir::DerivedFieldIr)
...
//! mutates the dense `&mut [Value]` array indexed by [`FieldId`](crate::base::FieldId).
//!
...
use crate::ir::{BuiltinId, DerivedFieldIr, LagAggregate, LinearNorm, Op, SymbolIdOrContinuous};
use chrono::Timelike;
...
thread_local! {
    static LAG_BUFFER: RefCell<HashMap<crate::base::FieldId, VecDeque<Value>>> = RefCell::new(HashMap::new());
}

#[allow(dead_code)]
fn lag_get(field: crate::base::FieldId, n: usize) -> Value {
    LAG_BUFFER.with(|buf| {
        let map = buf.borrow();
...
fn lag_push(field: crate::base::FieldId, val: Value) {
    LAG_BUFFER.with(|buf| {
...
/// use pmmlruntime::engine::transform::vm::{lag_update, lag_clear};
/// lag_clear();
...
/// lag_clear();
/// ```
pub fn lag_update(field: crate::base::FieldId, val: Value) {
    lag_push(field, val);
}
...
///
/// ```
/// use pmmlruntime::engine::transform::vm::lag_clear;
...
/// - `values`: Dense mutable array indexed by [`FieldId::as_usize`](crate::base::FieldId::as_usize). Must be at least
...
/// use pmmlruntime::ir::{DerivedFieldIr, Op, SymbolIdOrContinuous, BuiltinId};
/// use pmmlruntime::engine::transform::vm::{eval_derived_fields, lag_clear};
///
/// lag_clear();
...
pub fn eval_derived_fields(fields: &[DerivedFieldIr], values: &mut [Value]) -> Result<(), String> {
    ...
    for (idx, &val) in snapshot.iter().enumerate() {
        if !val.is_missing() {
            lag_push(crate::base::FieldId(idx as u32), val);
        }
    }
    for df in fields {
...
thread_local! {
    static SYMBOL_STR_MAP: RefCell<HashMap<crate::base::SymbolId, String>> = RefCell::new(HashMap::new());
}
/// Install the `SymbolId → String` map used to decode [`Value::Discrete`] for string builtins.
///
/// The VM stores discrete values as interned [`SymbolId`](crate::base::SymbolId)s; to evaluate
...
/// use pmmlruntime::engine::transform::vm::vm_set_symbol_map;
/// use std::collections::HashMap;
///
...
/// ```
pub fn vm_set_symbol_map(map: HashMap<crate::base::SymbolId, String>) {
    SYMBOL_STR_MAP.with(|m| *m.borrow_mut() = map);
}
fn discrete_to_string(sid: crate::base::SymbolId) -> String {
...
fn eval_lag(field: crate::base::FieldId, n: usize, aggregate: LagAggregate) -> Value {
                        ...
                        match id {
                            ...
                            _ => {
                                if let Some(s) = eval_string_builtin(*id, &strs) {
                                    ...
                                    s.hash(&mut h);
                                    let sid =
                                        crate::base::SymbolId((h.finish() & 0x7FFF_FFFF) as u32);
...
mod tests {
    use super::*;
    use crate::base::Value;
    use crate::ir::{DiscretizeBin, LinearNorm};
    ...
    fn builtin_arithmetic() {
        ...
        let bytecode = vec![
            Op::PushField(crate::base::FieldId(0)),
            Op::PushField(crate::base::FieldId(1)),
    ...
    fn discretize_bin() {
        let bins = vec![
            DiscretizeBin {
                bin_value: crate::base::SymbolId(10),
                interval_low: 0.0,
                interval_high: 10.0,
                left_closed: true,
                right_closed: false,
            ...
            DiscretizeBin {
                bin_value: crate::base::SymbolId(20),
                interval_low: 10.0,
                interval_high: 20.0,
                left_closed: true,
                right_closed: true,
        ...
        let res = eval_bytecode(&bytecode, &vals).unwrap();
        assert_eq!(res, Value::Discrete(crate::base::SymbolId(10)));
        let vals2 = vec![Value::Continuous(15.0)];
        let res2 = eval_bytecode(&bytecode, &vals2).unwrap();
        assert_eq!(res2, Value::Discrete(crate::base::SymbolId(20)));
    }

    #[test]
    fn mapvalues_lookup() {
        let table = vec![
            (crate::base::SymbolId(1), crate::base::SymbolId(100)),
            (crate::base::SymbolId(2), crate::base::SymbolId(200)),
        ];
        let bytecode = vec![
            Op::PushField(crate::base::FieldId(0)),
            Op::MapValues {
                table,
                default: Some(crate::base::SymbolId(999)),
            },
        ];
        let vals = vec![Value::Discrete(crate::base::SymbolId(1))];
        let res = eval_bytecode(&bytecode, &vals).unwrap();
        assert_eq!(res, Value::Discrete(crate::base::SymbolId(100)));
        let vals2 = vec![Value::Discrete(crate::base::SymbolId(3))];
        let res2 = eval_bytecode(&bytecode, &vals2).unwrap();
        assert_eq!(res2, Value::Discrete(crate::base::SymbolId(999)));
    }
    ...
    fn aggregate_sum() {
        let bytecode = vec![
            Op::PushField(crate::base::FieldId(0)),
            Op::PushField(crate::base::FieldId(1)),
            Op::PushField(crate::base::FieldId(2)),
    ...
    fn norm_discrete_basic() {
        let bytecode = vec![
            ...
            Op::NormDiscrete {
                field: crate::base::FieldId(0),
                value: crate::base::SymbolId(5),
                map_missing_to: None,
            },
        ];
        let vals = vec![Value::Discrete(crate::base::SymbolId(5))];
        let res = eval_bytecode(&bytecode, &vals).unwrap();
        assert_eq!(res, Value::Continuous(1.0));
        let vals2 = vec![Value::Discrete(crate::base::SymbolId(6))];
        let res2 = eval_bytecode(&bytecode, &vals2).unwrap();
    ...
    fn matches_regex() {
        let bytecode = vec![
            Op::PushField(crate::base::FieldId(0)),
            Op::PushConst(SymbolIdOrContinuous::Symbol(crate::base::SymbolId(999))),
    ...
    fn normalize_space() {
        ...
        let mut h = DefaultHasher::new();
        s.hash(&mut h);
        let sid = crate::base::SymbolId((h.finish() & 0x7FFF_FFFF) as u32);
