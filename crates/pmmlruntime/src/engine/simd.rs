...
/// indexed by [`FieldId`](crate::base::FieldId). For a single [`RegressionTableIr`](crate::ir::RegressionTableIr)
...
/// let out = pmmlruntime::engine::simd::evaluate_regression_batch_simd(&reg, &refs);
...
pub fn evaluate_regression_batch_simd(reg: &RegressionIr, batch_values: &[&[Value]]) -> Vec<Value> {
    if reg.regression_tables.len() != 1 {
        ...
        return batch_values
            .iter()
            .map(|vals| crate::engine::models::regression::evaluate_regression(reg, vals))
            .collect();
    }
    ...
    while i + 4 <= n {
        ...
        if has_categorical {
            ...
            for lane in 0..4 {
                ...
                let final_val = crate::engine::models::regression::apply_normalization(
                    lane_sum,
    ...
    // Remainder scalar
    while i < n {
        out.push(crate::engine::models::regression::evaluate_regression(
            reg,
            batch_values[i],
        ));
        i += 1;
...
///
/// Scoring is per-row via [`crate::engine::models::regression::evaluate_regression`] with identical
...
/// let refs: Vec<&[Value]> = rows.iter().map(|r| r.as_slice()).collect();
/// let out = pmmlruntime::engine::simd::evaluate_regression_batch_scalar(&reg, &refs);
