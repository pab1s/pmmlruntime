...
//! file-size cap, and nesting-depth cap. All unmarshaling in [`mod@crate::xml::unmarshal`]
...
//! model lowering live in [`mod@crate::xml::unmarshal`] and `pmml-ir`. Keeping the layer
...
//! to [`crate::xml::unmarshal::RawPmml`]. Mixing depth/file guards into each parse
...
//!
//! ```text
//! bytes: &[u8]  ──►  reader::PmmlReader / new_reader  ──►  unmarshal::unmarshal  ──►  RawPmml  ──►  pmmlruntime::ir::lower  ──►  Ir
//!                      (this module)                    (sibling)               (pmml-ir crate)
//! ```
//!
//! - `pmml-core` provides [`crate::base::PmmlError`] / [`crate::base::Result`] used for
...
//!   errors with [`crate::base::PmmlError::ValidationError`] when `depth > 512`. Matches
...
use crate::base::error::PmmlError;
...
/// # Examples
///
/// Basic construction and loop — mirrors how [`crate::xml::unmarshal::unmarshal`] uses it:
///
/// ```
...
impl<'a> PmmlReader<'a> {
    ...
    /// # Errors
    ///
    /// - [`crate::base::PmmlError::ValidationError`] if `bytes.len() > 100 MB`
    ...
    /// # Errors
    ///
    /// - [`crate::base::PmmlError::ValidationError`] when `depth > 512`
    ///   (`MAX_DEPTH`). Message contains `"depth"`.
    /// - [`crate::base::PmmlError::ParseError`] with `context: "xml"` when
