...
fn sequence_simple_rule() {
    ...
    let out2 = sess.run(&input2 as &dyn pmmlruntime::session::batch::Batch).unwrap().into_single().unwrap();
    assert_eq!(
    ...
    let out3 = sess.run(&input3 as &dyn pmmlruntime::session::batch::Batch).unwrap().into_single().unwrap();
...
fn bayesian_discrete_inference() {
    ...
    let out2 = sess.run(&HashMap::new() as &dyn pmmlruntime::session::batch::Batch).unwrap().into_single().unwrap();
...
<BayesianNetworkModel functionName="regression">
  ...
  <BayesianNetworkNodes>
    ...
    let out0 = sess.run(&in0 as &dyn pmmlruntime::session::batch::Batch).unwrap().into_single().unwrap();
    ...
    let out1 = sess.run(&in1 as &dyn pmmlruntime::session::batch::Batch).unwrap().into_single().unwrap();
